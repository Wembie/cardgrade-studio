...
export function analyzeCorners(card: ImageData, cornerSizePx?: number): CornerResult {
  ...
  const scores = corners.map(corner => {
    ...
    // Histogram peak: uniform border corners cluster 60%+ in one brightness bucket.
    // Artwork/full-bleed corners spread across buckets — can't measure wear.
    const BUCKET = 40
    ...
    if (maxBucket / cardPixels.length < 0.60) return 85

    const sorted = [...cardPixels].sort((a, b) => a - b)
    const median = sorted[Math.floor(sorted.length / 2)]

    // lo uses absolute offset (not median*0.75) to avoid false positives on dark borders
